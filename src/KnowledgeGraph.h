...
public:
    ...
    void generate_train_set(const std::string& relation_name, std::vector<std::pair<Node*, Node*> >& node_pairs, std::vector<int>& labels, 
        std::vector<Edge*>& forbiden_edges, size_t negative_samples_for_each_pair = 10) const;
    void generate_test_set(Edge* test_edge, size_t relation_id, bool change_src,
    ...
    void print_relation_statistics(std::ostream& os) const;
    //get
    ...
    void load_test(std::istream &is, std::vector<std::vector<Edge*> > &test_edge);
...
private:
    ...
    Edge * add_edge(size_t src_id, size_t dst_id, size_t relation_id);
};
